...
| `useScrollRestore` | Safety net for the scroll jump described under `page.tsx`. Watches `body[data-scroll-locked]`, snapshots the offset when a Radix overlay locks, then re-asserts it every frame for 1200ms after the lock clears. **The window must run to completion — do not add an early exit when the offset already matches.** `handleResendSuccess` awaits two IndexedDB writes, so the page shrinks *after* the dialog has closed; an early exit hands back control moments before the collapse and the net silently does nothing. It stops early only on real user intent (wheel/touch/key). |
